...
func SPrintStack(skip, max int) string {
	...
	var msg string
	msg = ""
	msg += fmt.Sprintf("Trace (most recent call last, max %d stack):\n", max)
	...
	for more == true && foundFrames <= max {
		...
		columns.Add(fmt.Sprintf("  %s:L%d", file, frame.Line), fmt.Sprintf(" @ %s", frame.Function))
		foundFrames++
	}
	return msg + columns.Reverse()
}
